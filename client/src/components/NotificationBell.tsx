...
const categories = [{ value: "all", label: "All" }, { value: "subscriptions", label: "Subs" }, { value: "appeals", label: "Appeals" }, { value: "verification", label: "Verification" }, { value: "social", label: "Social" }] as const;
...
function NotificationIcon({ type }: { type: string }) { if (type === "verification_reminder") return <ShieldCheck className="h-4 w-4 text-cyan-300" />; if (type === "subscribe") return <UserPlus className="h-4 w-4 text-indigo-300" />; if (type === "appeal_result") return <Sparkles className="h-4 w-4 text-amber-300" />; if (type === "comment") return <MessageCircle className="h-4 w-4 text-cyan-300" />; return <Bell className="h-4 w-4 text-slate-300" />; }
