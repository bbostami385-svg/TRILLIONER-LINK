...
const PUBLIC_PATHS = new Set(["/", "/login", "/signup", "/verify", "/welcome", "/mode-selection"]);
...
export function VerificationGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (authLoading || isPublicPath || !isAuthenticated || !user || ageStatus.isLoading || humanStatus.isLoading) return;
  ...
  }, [ageStatus.data, ageStatus.isLoading, authLoading, humanStatus.data, humanStatus.isLoading, isAuthenticated, isPublicPath, location, setLocation, user]);
