...
export function ReelsPage() {
  ...
  const shareToStory = trpc.stories.shareToStory.useMutation();
  ...
  const shareReelToStory = async (reel: NonNullable<typeof trendingReels>[number]) => { if (!isAuthenticated) { setLocation("/login"); return; } try { await shareToStory.mutateAsync({ mediaUrl: reel.videoUrl, mediaType: "video", caption: reel.caption || "TRILLIONER LINK Short", sourceType: "reel", sourceId: reel.id }); toast.success("Short added to your Story."); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not share this Short to your Story."); } };
            ...
            <div className="mx-auto max-w-2xl space-y-6">
              {trendingReels.map((reel) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2"><ReactionPicker compact selected={selectedReactions[reel.id]} onSelect={(reaction) => chooseReelReaction(reel.id, reaction)} onLike={() => void handleLikeReel(reel.id)} /><VideoShareMenu title={reel.caption || "TRILLIONER LINK Short"} text={reel.caption || undefined} url={`${window.location.origin}/shorts?video=${reel.id}`} /><button type="button" aria-label="Share Short to Story" onClick={() => void shareReelToStory(reel)} disabled={shareToStory.isPending} className="grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white transition hover:bg-white/20 disabled:opacity-50">{shareToStory.isPending ? "…" : "✦"}</button><button type="button" aria-label={watchLaterSaved[reel.id] ? "Remove Short from Watch Later" : "Save Short to Watch Later"} onClick={() => saveReelToWatchLater(reel)} className={`grid h-10 w-10 place-items-center rounded-full transition ${watchLaterSaved[reel.id] ? "bg-amber-300 text-slate-950" : "bg-white/10 text-white hover:bg-white/20"}`}><Clock3 className="h-4 w-4" /></button></div>
                </Card>
              ))}
            </div>
          ) : (
