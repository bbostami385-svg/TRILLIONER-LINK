...
export function removeWatchedWatchLater(isWatched: (item: WatchLaterItem) => boolean) { const next = readItems().filter((item) => !isWatched(item)); writeItems(next); return next; }
