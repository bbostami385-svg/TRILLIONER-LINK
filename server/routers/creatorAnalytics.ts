...
export const creatorAnalyticsRouter = router({
  ...
  getOverview: protectedProcedure.input(analyticsRangeSchema).query(async ({ ctx, input }) => {
    ...
    const videoRows = await db.select({ createdAt: videos.createdAt, title: videos.title, category: videos.category, hashtags: videos.hashtags, duration: videos.duration, views: videos.views, likes: videos.likes, comments: videos.comments }).from(videos).where(videoWhere(ctx.user.id, start, end, input.category, input.hashtag)).orderBy(desc(videos.createdAt));
    ...
    const estimatedWatchTimeSeconds = videoRows.reduce((sum, row) => sum + Number(row.duration ?? 0) * Number(row.views), 0);
    ...
    const videoPerformance = videoRows.map((video, index) => ({ id: index + 1, title: video.title, category: video.category, hashtags: video.hashtags ?? [], durationSeconds: Number(video.duration ?? 0), views: Number(video.views), likes: Number(video.likes), comments: Number(video.comments), estimatedWatchTimeSeconds: Number(video.duration ?? 0) * Number(video.views), engagement: Number(video.likes) + Number(video.comments), createdAt: video.createdAt }));
